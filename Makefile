...
deploy: ## Deploy ALL stack
	cdk deploy cognito-identity-provider --profile ${AWS_PROFILE} --require-approval never
	cdk deploy premium-content-provider --profile ${AWS_PROFILE} --require-approval never
	cdk deploy content-consumers-stack --profile ${AWS_PROFILE} --require-approval never

destroy: ## Delete Stack without confirmation
	cdk destroy cognito-identity-provider --profile ${AWS_PROFILE} --force
	cdk destroy premium-content-provider --profile ${AWS_PROFILE} --force
	cdk destroy content-consumers-stack --profile ${AWS_PROFILE} --force
